...
async function runTests() {
    console.log(`${colors.bold}${colors.blue}🚀 Starting Golden Test Set Execution...${colors.reset}\n`);
    ...
    try {
        ...
        console.log(`Loaded Golden Set v${goldenSet.version} with ${goldenSet.tests.length} tests.`);
        console.log(`Target API: ${API_URL}\n`);
    } catch (error) {
        console.error(`${colors.red}Failed to load golden-test-set.json: ${error}${colors.reset}`);
        process.exit(1);
    }
    ...
    for (const test of goldenSet.tests) {
        console.log(`${colors.yellow}Running Test #${test.id}: ${test.question}${colors.reset}`);
        ...
        try {
            const response = await axios.post(
                ...
                {
                    headers: { 'X-Bypass-Cache': 'true', 'Content-Type': 'application/json' },
            ...
            const expectedTypes = Array.isArray(test.expectedResponseType) ? test.expectedResponseType : [test.expectedResponseType];
            ...
            if (passed && test.verdictQuestion) {
                const hasAlinti = userResponse.includes('**ALINTI**') || userResponse.includes('**QUOTE**'); // Simple check
                // const hasHukum = userResponse.toLowerCase().includes('hüküm cümlesi seçilemedi');

                if (!hasAlinti && !userResponse.toLowerCase().includes('hüküm cümlesi seçilemedi')) {
            ...
            const result: TestResult = {
                ...
                debugInfo: { responseType: actualResponseType, sourcesCount, earlyExit: debug.earlyExit }
            };

            results.push(result);

            if (passed) {
                console.log(`${colors.green}✅ PASS${colors.reset} (${actualResponseType})`);
                passedCount++;
            } else {
                console.log(`${colors.red}❌ FAIL${colors.reset} - ${failureReason}`);
                console.log(`   Response Preview: ${userResponse.substring(0, 100)}...`);
                failedCount++;
            }

        } catch (error: any) {
            console.error(`${colors.red}❌ ERROR${colors.reset} - API Call Failed: ${error.message}`);
            results.push({
                id: test.id,
                question: test.question,
                passed: false,
