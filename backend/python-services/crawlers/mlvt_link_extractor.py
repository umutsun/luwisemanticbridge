...
# -*- coding: utf-8 -*-
"""
Ministry of Labour and Vocational Training (MLVT) Link Extractor
Extracts all labor and work permit related links from mlvt.gov.kh

Categories:
- Labor Law
- Foreign worker quotas (10% rule)
- Work permit requirements
- Minimum wage and severance pay (NSSF)
"""
...
# MLVT Categories with their base URLs and patterns
MLVT_CATEGORIES = {
    'labor_law': {
        'name': 'Labor Law',
        'base_url': 'https://mlvt.gov.kh',
        'list_url': 'https://mlvt.gov.kh/en/laws',
        'link_pattern': r'/en/laws',
        'output_file': 'MLVT-LABOR_LAW_LINKLERI.json'
    },
    'foreign_workers': {
        'name': 'Foreign Worker Quotas (10% Rule)',
        'base_url': 'https://mlvt.gov.kh',
        'list_url': 'https://mlvt.gov.kh/en/foreign-workers',
        'link_pattern': r'/en/foreign-workers',
        'output_file': 'MLVT-FOREIGN_WORKERS_LINKLERI.json'
    },
    'work_permit': {
        'name': 'Work Permit Requirements',
        'base_url': 'https://mlvt.gov.kh',
        'list_url': 'https://mlvt.gov.kh/en/work-permit',
        'link_pattern': r'/en/work-permit',
        'output_file': 'MLVT-WORK_PERMIT_LINKLERI.json'
    },
    'minimum_wage': {
        'name': 'Minimum Wage and NSSF',
        'base_url': 'https://mlvt.gov.kh',
        'list_url': 'https://mlvt.gov.kh/en/wages',
        'link_pattern': r'/en/wages',
        'output_file': 'MLVT-MINIMUM_WAGE_LINKLERI.json'
    }
}
...
async def extract_category_links(category_key: str) -> dict:
    ...
    if category_key not in MLVT_CATEGORIES:
        print(f"[ERROR] Unknown category: {category_key}")
        return {'links': [], 'count': 0}

    config = MLVT_CATEGORIES[category_key]
...
async def main():
    """Main function to extract all MLVT categories"""
    print("=" * 60)
    print("Ministry of Labour and Vocational Training (MLVT) Link Extractor")
    print("=" * 60)

    # Extract all categories
    results = {}
    for category_key in MLVT_CATEGORIES.keys():
    ...
    for category_key, result in results.items():
        print(f"{MLVT_CATEGORIES[category_key]['name']}: {result['count']} links")
