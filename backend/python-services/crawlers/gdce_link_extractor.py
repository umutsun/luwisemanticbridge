...
# -*- coding: utf-8 -*-
"""
General Department of Customs and Excise (GDCE) Link Extractor
Extracts all customs and import/export related links from customs.gov.kh

Categories:
- Customs tariffs
- Import/export procedures
- Spare parts and equipment import exemptions
"""
...
# GDCE Categories with their base URLs and patterns
GDCE_CATEGORIES = {
    'customs_tariffs': {
        'name': 'Customs Tariffs',
        'base_url': 'https://customs.gov.kh',
        'list_url': 'https://customs.gov.kh/en/customs/tariffs',
        'link_pattern': r'/en/customs/tariffs',
        'output_file': 'GDCE-CUSTOMS_TARIFFS_LINKLERI.json'
    },
    'import_procedures': {
        'name': 'Import Procedures',
        'base_url': 'https://customs.gov.kh',
        'list_url': 'https://customs.gov.kh/en/customs/import',
        'link_pattern': r'/en/customs/import',
        'output_file': 'GDCE-IMPORT_PROCEDURES_LINKLERI.json'
    },
    'export_procedures': {
        'name': 'Export Procedures',
        'base_url': 'https://customs.gov.kh',
        'list_url': 'https://customs.gov.kh/en/customs/export',
        'link_pattern': r'/en/customs/export',
        'output_file': 'GDCE-EXPORT_PROCEDURES_LINKLERI.json'
    },
    'exemptions': {
        'name': 'Import Exemptions (Spare Parts & Equipment)',
        'base_url': 'https://customs.gov.kh',
        'list_url': 'https://customs.gov.kh/en/customs/exemptions',
        'link_pattern': r'/en/customs/exemptions',
        'output_file': 'GDCE-EXEMPTIONS_LINKLERI.json'
    }
}
            ...
            try:
                ...
                if href:
                    ...
                    if re.search(category_config['link_pattern'], href):
                        # Normalize URL
                        if href.startswith('/'):
                            href = f"https://customs.gov.kh{href}"
...
async def extract_category_links(category_key: str) -> dict:
    ...
    if category_key not in GDCE_CATEGORIES:
        print(f"[ERROR] Unknown category: {category_key}")
        return {'links': [], 'count': 0}

    config = GDCE_CATEGORIES[category_key]
...
async def main():
    """Main function to extract all GDCE categories"""
    print("=" * 60)
    print("General Department of Customs and Excise (GDCE) Link Extractor")
    print("=" * 60)

    # Extract all categories
    results = {}
    for category_key in GDCE_CATEGORIES.keys():
    ...
    for category_key, result in results.items():
        print(f"{GDCE_CATEGORIES[category_key]['name']}: {result['count']} links")
