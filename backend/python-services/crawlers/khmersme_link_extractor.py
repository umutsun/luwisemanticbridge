...
# -*- coding: utf-8 -*-
"""
KhmerSME (KOBİ ve İşletme Portalı) Link Extractor
Extracts all SME and business setup links from khmersme.gov.kh
...
# KhmerSME Categories with their base URLs and patterns
KHMER_SME_CATEGORIES = {
    'business_setup': {
        'name': 'Business Setup Requirements',
        'base_url': 'https://khmersme.gov.kh',
        'list_url': 'https://khmersme.gov.kh/en/business-setup',
        'link_pattern': r'/en/business-setup',
        'output_file': 'KHMER-SME-BUSINESS_SETUP_LINKLERI.json'
    },
    'licensing': {
        'name': 'Licensing Requirements',
        'base_url': 'https://khmersme.gov.kh',
        'list_url': 'https://khmersme.gov.kh/en/licensing',
        'link_pattern': r'/en/licensing',
        'output_file': 'KHMER-SME-LICENSING_LINKLERI.json'
    },
    'procedures': {
        'name': 'Step-by-Step Procedures',
        'base_url': 'https://khmersme.gov.kh',
        'list_url': 'https://khmersme.gov.kh/en/procedures',
        'link_pattern': r'/en/procedures',
        'output_file': 'KHMER-SME-PROCEDURES_LINKLERI.json'
    },
    'guides': {
        'name': 'Business Guides',
        'base_url': 'https://khmersme.gov.kh',
        'list_url': 'https://khmersme.gov.kh/en/guides',
        'link_pattern': r'/en/guides',
        'output_file': 'KHMER-SME-GUIDES_LINKLERI.json'
    }
}
            ...
            try:
                ...
                if href:
                    ...
                    if re.search(category_config['link_pattern'], href):
                        # Normalize URL
                        if href.startswith('/'):
                            href = f"https://khmersme.gov.kh{href}"
...
async def extract_category_links(category_key: str) -> dict:
    ...
    if category_key not in KHMER_SME_CATEGORIES:
        print(f"[ERROR] Unknown category: {category_key}")
        return {'links': [], 'count': 0}

    config = KHMER_SME_CATEGORIES[category_key]
...
async def main():
    """Main function to extract all KhmerSME categories"""
    print("=" * 60)
    print("KhmerSME (KOBİ ve İşletme Portalı) Link Extractor")
    print("=" * 60)

    # Extract all categories
    results = {}
    for category_key in KHMER_SME_CATEGORIES.keys():
    ...
    for category_key, result in results.items():
        print(f"{KHMER_SME_CATEGORIES[category_key]['name']}: {result['count']} links")
