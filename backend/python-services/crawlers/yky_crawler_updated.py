...
def save_state(queue, visited, crawler_name=None, script_name=None):
    ...
    try:
        ...
        if crawler_name:
            ...
            print(f"[STATE] Saved: {len(visited)} visited, {len(queue)} queued (file + Redis){script_info}")
        else:
            print(f"[STATE] Saved: {len(visited)} visited, {len(queue)} queued (file only)")
    except Exception as e:
...
async def main():
    ...
    # Parse arguments: URL, crawler_name, script_name (optional)
    if len(sys.argv) < 3:
        print("Usage: python yky_crawler.py <url> <crawler_name> [script_name]")
        print("\nExamples:")
        print("  python yky_crawler.py https://kitap.ykykultur.com.tr/kitaplar/konu-dizini/dogan-kardes yky_crawler")
        print("  python yky_crawler.py https://kitap.ykykultur.com.tr/kitaplar/konu-dizini/dogan-kardes yky_crawler yky_crawler.py")
        sys.exit(1)

    start_url_param = sys.argv[1]
    ...
    print(f"[INIT] Start URL: {start_url_param}")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            while queue:
                ...
                # Save state periodically (every page)
                if len(visited) % 10 == 0:
