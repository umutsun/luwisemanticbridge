...
# -*- coding: utf-8 -*-
"""
Open Development Cambodia (ODC) Link Extractor
Extracts all legislation links from opendevelopmentcambodia.net

Categories:
- Laws (Chbab)
- Royal Decrees (Reach Kret)
- Sub-decrees (Anukret)
- Ministry Directives (Prakas)
"""
...
# ODC Categories with their base URLs and patterns
ODC_CATEGORIES = {
    'laws': {
        'name': 'Laws (Chbab)',
        'base_url': 'https://opendevelopmentcambodia.net/laws/',
        'list_url': 'https://opendevelopmentcambodia.net/laws/',
        'link_pattern': r'/laws/',
        'output_file': 'ODC-LAWS_LINKLERI.json'
    },
    'royal_decrees': {
        'name': 'Royal Decrees (Reach Kret)',
        'base_url': 'https://opendevelopmentcambodia.net/laws/',
        'list_url': 'https://opendevelopmentcambodia.net/laws/',
        'link_pattern': r'/royal-decrees/',
        'output_file': 'ODC-ROYAL_DECREES_LINKLERI.json'
    },
    'sub_decrees': {
        'name': 'Sub-decrees (Anukret)',
        'base_url': 'https://opendevelopmentcambodia.net/laws/',
        'list_url': 'https://opendevelopmentcambodia.net/laws/',
        'link_pattern': r'/sub-decrees/',
        'output_file': 'ODC-SUB_DECREES_LINKLERI.json'
    },
    'directives': {
        'name': 'Ministry Directives (Prakas)',
        'base_url': 'https://opendevelopmentcambodia.net/laws/',
        'list_url': 'https://opendevelopmentcambodia.net/laws/',
        'link_pattern': r'/prakas/',
        'output_file': 'ODC-DIRECTIVES_LINKLERI.json'
    }
}
            ...
            try:
                ...
                if href:
                    ...
                    if re.search(category_config['link_pattern'], href):
                        # Normalize URL
                        if href.startswith('/'):
                            href = f"https://opendevelopmentcambodia.net{href}"
...
async def extract_category_links(category_key: str) -> dict:
    ...
    if category_key not in ODC_CATEGORIES:
        print(f"[ERROR] Unknown category: {category_key}")
        return {'links': [], 'count': 0}

    config = ODC_CATEGORIES[category_key]
...
async def main():
    """Main function to extract all ODC categories"""
    print("=" * 60)
    print("Open Development Cambodia (ODC) Link Extractor")
    print("=" * 60)

    # Extract all categories
    results = {}
    for category_key in ODC_CATEGORIES.keys():
    ...
    for category_key, result in results.items():
        print(f"{ODC_CATEGORIES[category_key]['name']}: {result['count']} links")
