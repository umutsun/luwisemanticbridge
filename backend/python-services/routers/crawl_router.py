...
class SahibindenCrawlRequest(BaseModel):
    """Request model for Sahibinden crawling"""
    url: str = Field(..., description="Sahibinden listing URL (e.g., https://www.sahibinden.com/arsa/izmir)")
    max_pages: int = Field(3, ge=1, le=20, description="Maximum pages to crawl")

class SahibindenCrawlResponse(BaseModel):
    """Response model for Sahibinden crawl"""
    success: bool
    job_id: str
    message: str
    url: str
...
# Store running jobs
_sahibinden_jobs: Dict[str, Dict[str, Any]] = {}

@router.post("/sahibinden", response_model=SahibindenCrawlResponse)
async def crawl_sahibinden(
    request: SahibindenCrawlRequest,
    background_tasks: BackgroundTasks
) -> SahibindenCrawlResponse:
    """
    Crawl Sahibinden listings

    Uses Patchright (stealth Playwright) to bypass Cloudflare Turnstile.
    Creates fresh browser context for each page to avoid session tracking.
    """
    import uuid
    job_id = str(uuid.uuid4())[:8]

    # Store job status
    _sahibinden_jobs[job_id] = {
        "status": "queued",
        "url": request.url,
    ...
    # Add to background tasks
    background_tasks.add_task(
        _run_sahibinden_crawler,
        job_id,
        request.url,
        request.max_pages
    )

    logger.info(f"[SAHIBINDEN] Job {job_id} queued: {request.url}")

    return SahibindenCrawlResponse(
        success=True,
        job_id=job_id,
        message=f"Crawl job started. Check status with /crawl/sahibinden/status/{job_id}",
        url=request.url,
        max_pages=request.max_pages
    )

async def _run_sahibinden_crawler(job_id: str, url: str, max_pages: int):
    """Run Sahibinden crawler in background"""
    ...
    try:
        _sahibinden_jobs[job_id]["status"] = "running"
        _sahibinden_jobs[job_id]["started_at"] = datetime.now().isoformat()

        logger.info(f"[SAHIBINDEN] Job {job_id} starting crawler...")

        # Import and run crawler
        from sahibinden_list_crawler import SahibindenListCrawler

        crawler = SahibindenListCrawler(url, max_pages=max_pages)
        listings = await crawler.crawl()

        _sahibinden_jobs[job_id]["status"] = "completed"
        _sahibinden_jobs[job_id]["completed_at"] = datetime.now().isoformat()
        _sahibinden_jobs[job_id]["listings_count"] = len(listings)

        logger.info(f"[SAHIBINDEN] Job {job_id} completed: {len(listings)} listings")

    except Exception as e:
        logger.error(f"[SAHIBINDEN] Job {job_id} failed: {e}")
        _sahibinden_jobs[job_id]["status"] = "failed"
        _sahibinden_jobs[job_id]["error"] = str(e)
        _sahibinden_jobs[job_id]["completed_at"] = datetime.now().isoformat()

@router.get("/sahibinden/status/{job_id}")
async def get_sahibinden_status(job_id: str) -> Dict[str, Any]:
    """Get Sahibinden crawl job status"""
    if job_id not in _sahibinden_jobs:
        raise HTTPException(status_code=404, detail="Job not found")

    return _sahibinden_jobs[job_id]

@router.get("/sahibinden/jobs")
async def list_sahibinden_jobs() -> Dict[str, Any]:
    """List all Sahibinden crawl jobs"""
    return {
        "jobs": _sahibinden_jobs,
        "count": len(_sahibinden_jobs)
    }