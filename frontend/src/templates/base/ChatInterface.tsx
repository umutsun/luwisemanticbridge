...
export default function ChatInterface() {
  ...
  // FORCE Base theme (classic design)
  const theme = useTheme('base');

  // Base-specific features (detailed sources, keyword highlighting enabled)
  ...
  // Shuffle suggestions once and keep stable
  ...
  const handleSendMessage = async (fromSource: boolean = false) => {
    ...
    try {
      ...
      let userFriendlyMessage = 'Üzgünüm, şu anda yanıt veremiyorum.';

      if (errorMessage.includes(': 429') || errorMessage.includes('QUERY_LIMIT_EXCEEDED')) {
        userFriendlyMessage = user?.role === 'admin'
          ? 'Admin kullanıcılarsınız ancak teknik bir sorun oluştu.'
          : 'Aylık soru limitinizi doldurdunuz.';
      }
