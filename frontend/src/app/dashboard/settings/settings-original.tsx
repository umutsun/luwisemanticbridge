...
function LLMSettings() {
    ...
    try {
      ...
      const defaultConfig = {
        ...
        anthropic: {
          ...
          model: data?.anthropic?.model || 'claude-3-opus-20240229',
        ...
        deepseek: {
          ...
          model: data?.deepseek?.model || 'deepseek-coder'
        },
        huggingface: {
          ...data?.huggingface,
      ...
      // Load saved token info and API status if available
      if (data?.tokenInfo) {
  ...
  const saveAllSettings = async () => {
    setSaving(true);
    try {
      // Save all data including token info and API status
  ...
  const validateApiKey = async (provider: string, apiKey: string) => {
    ...
    try {
      ...
      const tokenInfoData = {
        ...
        model: result.model || getDefaultModelForProvider(provider),
      ...
      toast({
        title: "Success",
        description: `${provider} API key validated (${result.model || getDefaultModelForProvider(provider)}, ${result.responseTime || responseTime}ms)`,
      });
    } catch (error) {
  ...
  const getModelsForProvider = (provider: string) => {
    const models: Record<string, string[]> = {
      ...
      google: ['gemini-1.5-flash-latest', 'gemini-1.5-pro-latest', 'gemini-pro', 'gemini-pro-vision'],
            ...
            <div key={provider} className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${
                    providerStatus.status === 'active' ? 'bg-green-500' :
                    providerStatus.status === 'error' ? 'bg-red-500' :
                    'bg-gray-300'
                  }`} />
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={() => validateApiKey(provider, tempConfig?.[provider]?.apiKey || '')}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="openai" disabled={!isProviderValidated('openai')}>
                      OpenAI {isProviderValidated('openai') && '✓'}
                    </SelectItem>
                    <SelectItem value="google" disabled={!isProviderValidated('google')}>
                      Google {isProviderValidated('google') && '✓'}
                    </SelectItem>
                    <SelectItem value="anthropic" disabled={!isProviderValidated('anthropic')}>
                      Anthropic {isProviderValidated('anthropic') && '✓'}
                    </SelectItem>
                    <SelectItem value="deepseek" disabled={!isProviderValidated('deepseek')}>
                      DeepSeek {isProviderValidated('deepseek') && '✓'}
                    </SelectItem>
                    <SelectItem value="huggingface" disabled={!isProviderValidated('huggingface')}>
                      HuggingFace {isProviderValidated('huggingface') && '✓'}
                    </SelectItem>
                    <SelectItem value="openrouter" disabled={!isProviderValidated('openrouter')}>
                      OpenRouter {isProviderValidated('openrouter') && '✓'}
                    </SelectItem>
                  </SelectContent>
                </Select>
                <Select
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="deepl" disabled={!isProviderValidated('deepl')}>
                    DeepL {isProviderValidated('deepl') && '✓'}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
