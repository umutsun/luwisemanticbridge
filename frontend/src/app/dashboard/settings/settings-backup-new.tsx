...
function LLMSettings() {
  ...
  const validateApiKey = async (provider: string, apiKey: string, model?: string) => {
    ...
    try {
      ...
      // Use the selected model for API validation
      const selectedModel = model || tempConfig?.[provider]?.model || getDefaultModelForProvider(provider);

      // Call the real backend API validation endpoint
      ...
      // Store real token usage info from backend
      const tokenInfoData = {
        used: result.usage?.totalTokens || result.usage?.inputTokens + result.usage?.outputTokens || 0,
      ...
      // Store detailed API status from real response
      setApiStatus(prev => ({
        ...prev,
        [provider]: {
          status: 'active',
      ...
      await updateSettingsCategory('llm', {
        ...
        apiStatus: { ...apiStatus, [provider]: apiStatus[provider] },
        modelTokenUsage: { ...modelTokenUsage, [modelKey]: modelTokenUsage[modelKey] }
      });

      toast({
        title: "Success",
        description: `${provider} API key validated (${result.model || selectedModel}, ${result.responseTime || responseTime}ms)`,
      });
    } catch (error) {
  ...
  const validateAllModelsForProvider = async (provider: string, apiKey: string) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `${provider} API validated (${successfulModels.length}/${models.length} models successful)`,
      });
    } catch (error) {
      // Store error status with reason
      setApiStatus(prev => ({
  ...
  const isProviderValidated = (provider: string) => {
    return validatedKeys.has(provider) && llmConfig?.[provider]?.apiKey && llmConfig[provider].apiKey !== '••••••••';
  };
            ...
            <div key={provider} className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <Button
                  size="sm"
                  variant={isProviderValidated(provider) ? "default" : "outline"}
              ...
              {(tokenInfo[provider] || providerStatus?.status === 'error') && (
                  ...
                  {providerStatus.status === 'active' && tokenInfo[provider] ? (
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">
                            ({providerStatus.lastChecked?.toLocaleTimeString()})
                          </span>
                        </div>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Model:</span>
                        <span className="font-medium">{providerStatus.model || tokenInfo[provider].model}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Response Time:</span>
                        <span className="font-medium">{providerStatus.responseTime}ms</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Cost per Request:</span>
                        <span className="font-medium">${tokenInfo[provider].cost.toFixed(4)}</span>
                      </div>
                      {tokenInfo[provider].translationProvider ? (
                        <>
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">Characters Used:</span>
                            <span className="font-medium">{tokenInfo[provider].charsUsed.toLocaleString()} / {tokenInfo[provider].charsLimit.toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">Remaining:</span>
                            <span className="font-medium text-green-600">{(tokenInfo[provider].charsLimit - tokenInfo[provider].charsUsed).toLocaleString()}</span>
                          </div>
                          <div className="w-full bg-background rounded-full h-2 mt-2">
                            <div
                              className="bg-blue-500 h-2 rounded-full transition-all duration-300"
                              style={{ width: `${(tokenInfo[provider].charsUsed / tokenInfo[provider].charsLimit) * 100}%` }}
                            />
                          </div>
                        </>
                      ) : (
                        <>
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{tokenInfo[provider].used.toLocaleString()} / {tokenInfo[provider].limit.toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">Remaining:</span>
                            <span className="font-medium text-green-600">{(tokenInfo[provider].limit - tokenInfo[provider].used).toLocaleString()}</span>
                          </div>
                          <div className="w-full bg-background rounded-full h-2 mt-2">
                            <div
                              className="bg-primary h-2 rounded-full transition-all duration-300"
                              style={{ width: `${(tokenInfo[provider].used / tokenInfo[provider].limit) * 100}%` }}
                            />
                          </div>
                        </>
                      )}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-red-600 text-xs text-right max-w-[60%]">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {providerStatus.lastChecked?.toLocaleString()}
                        </span>
                      </div>
                    </div>
                  ) : null}
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  {isProviderValidated('google') && (
                    <SelectItem value="google">Google ✓</SelectItem>
                  )}
                  {isProviderValidated('openai') && (
                    <SelectItem value="openai">OpenAI ✓</SelectItem>
                  )}
                  {isProviderValidated('anthropic') && (
                    <SelectItem value="anthropic">Anthropic ✓</SelectItem>
                  )}
                  {isProviderValidated('deepl') && (
...
function RAGSettings() {
  ...
  const loadSettings = useCallback(async () => {
    setLoading(true);
    try {
      const [ragData, chatbotData, appSettingsData] = await Promise.all([
        getRAGSettings(),
        getSettingsCategory('chatbot'),
        getAppSettings()
      ]);

      console.log('App Settings Response:', appSettingsData);
      console.log('LLM Settings from App:', appSettingsData?.llmSettings);
      console.log('System Prompt from App:', appSettingsData?.llmSettings?.systemPrompt);

      setRagConfig(ragData);
      setTempRAGConfig(ragData);
      setChatbotConfig(chatbotData);
      setTempChatbotConfig(chatbotData);
      setLlmConfig(appSettingsData || {});
    } catch (error) {
  ...
  const saveAllSettings = async () => {
    setSaving(true);
    try {
      // Update system prompt in LLM settings if it was changed
      if (tempRAGConfig?.ragSettings?.systemPrompt !== llmConfig?.llmSettings?.systemPrompt) {
        await updateSettingsCategory('llm', {
          llmSettings: {
            ...llmConfig?.llmSettings,
            systemPrompt: tempRAGConfig?.ragSettings?.systemPrompt
          }
        });
      }
      ...
      // Reload LLM config to get updated system prompt
      const updatedLLMData = await getLLMSettings();
      setLlmConfig(updatedLLMData);

      toast({
        title: "Success",
        description: "Settings saved successfully. System prompt updated in database.",
      });
    } catch (error) {
      toast({
        title: "Error",
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <Label>Active System Prompt (from Database)</Label>
              <Textarea
                value={llmConfig?.llmSettings?.systemPrompt || llmConfig?.systemPrompt || llmConfig?.llm_system_prompt || tempRAGConfig?.ragSettings?.systemPrompt || ragConfig?.ragSettings?.systemPrompt || 'Sen bir RAG asistanısın. SADECE verilen context\'ten cevap ver. Context dışında bilgi verme.'}
                placeholder="Active system prompt from database"
                rows={4}
                onChange={(e) => updateRAGSetting('systemPrompt', e.target.value)}
                className="mt-2"
              />
              <p className="text-xs text-muted-foreground mt-1">
                This is the active system prompt currently being used by the LLM. It's loaded from the database (llm_system_prompt setting).
              </p>
            </div>
